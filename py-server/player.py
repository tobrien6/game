...
class Player:
    def __init__(self, player_id, user_id, name, websocket, event_queue, 
                 x=0, y=0, health=100, action_points=10.0, last_update_time=None):
    ...
    @classmethod
    def create_new_player(self, user_id, name, websocket, event_queue):
    ...
    async def update_action_points(self, amount=None):
        ...
        if amount and amount < 0:
            # if a player uses ap, send an event to the event queue to distribute ap to nearby players
            event = {'type': EventType.PLAYER_AP_USAGE,
                     'data': {'action': 'PlayerAPUsage', 'player': self, 'ap_used': amount}}
            await self.event_queue.put_event(event)

    async def move_to_tile(self, tile_x, tile_y, world_grid, n_entities_near):
        print(n_entities_near)
        ...
        if self.action_points >= -move_cost:
            ...
            self.x = tile_x
            self.y = tile_y
            await self.update_action_points(move_cost)
            print("Player moved to tile ({}, {})".format(tile_x, tile_y))
        elif len(n_entities_near) == 0:
            # Allow the player to move for free if there are no entities nearby
            self.x = tile_x
            self.y = tile_y
            await self.update_action_points()
        else:
